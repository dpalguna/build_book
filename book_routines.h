...
//void read_and_update(const char& a_typeOfMessage, char* a_buffer, int& a_buffer_index, map<uint32_t, map<uint64_t, order> >&a_BB,  map<uint32_t, map<uint64_t, order> >&a_AB,  map<uint64_t, order>& a_OID_to_properties, globalTime& a_globalTime);
//void read_next_message(const char& a_typeOfMessage, char* a_buffer, int& a_buffer_index, globalTime& a_globalTime);
