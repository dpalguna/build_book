     ...
     while(!end_of_day)
      {
       ...
       if(start_of_trading && o_globalTime.second < FOUR_PM && o_nextGlobalTime.second < FOUR_PM)
        {
    	   ...
    	   if(o_globalTime.timeStampToMs() != o_nextGlobalTime.timeStampToMs())
             {
               ...
               for(int i = 0; i< numMultiples; i++)
                 {
                   closingBooks[closingBookIndex].initialize(lob.bidShares, lob.askShares, lob.bidPrice, lob.askPrice);
